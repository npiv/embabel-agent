...
WARNING: (jasper notes) What do the keys in the Map.of("foo" to bar) do? For the point above about MCP servers, let's link to where we can read about it? (Even if stubbed for now).